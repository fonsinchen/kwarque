    ...
    exports.init = function(args) {
        ...
        node = function (x, y, w, h, depth, maxChildren, maxDepth) {
            ...
            return {
                ...
                dump : function() {
                    console.log('node', this.depth, this.x, this.y, this.w, this.h)
                    console.log('children', children);
                    if (nodes.length) {
                        for (var i = 0; i < PARENT; ++i) nodes[i].dump();
                    } else {
                ...
                clean : function (maxdepth) {
                    if (typeof maxdepth === 'undefined') maxdepth = 1;
                    if (maxdepth === 0) return nodes.length === 0 && children.length === 0;
                    var empty = true;
                    if (nodes.length > 0) {
                        for (var i = 0; i < PARENT; ++i) {
                            empty = empty && nodes[i].clean(maxdepth - 1);
                        }
                        if (empty) nodes.length = 0;
                    }
                    return empty && children.length === 0;
                },
                ...
                remove : function (item) {
                    ...
                    if (nodes.length) {
                        ...
                        i = this.findInsertNode(item)
                        if (i === PARENT) {
                            return children.splice(children.indexOf(item), 1)[0];
                        } else {
                            return nodes[i].remove(item);
                        }
                    } else {
                        return children.splice(children.indexOf(item), 1)[0];
                    }
                 ...
                 * be larger twice the area's, except if maxDepth is reached.
                 * Call the callback with each of those nodes.
                 */
                prepare : function (area, callback) {
                    if (this.depth === maxDepth || 2 * area.w > this.w || 2 * area.h > this.h) {
                        callback(this);
                    } else {
                        if (!nodes.length) this.divide();
                        this.findOverlappingNodes(area, function(dir) {
                            nodes[dir].prepare(area, callback);
                        });
                    }
                },

                /**
                 * Make sure an equivalent to the given node is present in the
                 * tree and return it.
                 */
                insertNode : function (node) {
                    if (this.depth < node.depth) {
                        if (!nodes.length) this.divide();
                        var i = this.findInsertNode(node);
                        return nodes[i].insertNode(node);
                    } else {
                        return this;
                    }
                },
                
                retrieveNodes : function (item, callback) {
                    // check if node has subnodes
                    if (nodes.length) {
                        // call retrieve on all matching subnodes
                        this.findOverlappingNodes(item, function(dir) {
                            nodes[dir].retrieveNodes(item, callback);
                        });
                    }
                    callback(this);
                }
        ...
        return {
            ...
            prepare : function (area, callback) {
                this.root.prepare(area, callback);
            },
            ...
            retrieveNodes : function (item, callback) {
                this.root.retrieveNodes(item, callback);
            },
