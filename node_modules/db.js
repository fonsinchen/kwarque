...
kwarque.on('clientConnect', function(client) {
    ...
    client.on("watch", function(select, fn) {
        ...
        for (var i = 0; i < client.lon.length - 1; ++i) {
            for (var j = 0; j < client.lat.length - 1; ++j) {
                client.leave('#' + client.lon[i] + '_' + client.lat[j]);
            }
        }
        client.lon = raster.transform('lon', select.lon, raster.messageLevel('lon')).values;
        client.lat = raster.transform('lat', select.lat, raster.messageLevel('lat')).values;
        for (i = 0; i < client.lon.length - 1; ++i) {
            for (j = 0; j < client.lat.length - 1; ++j) {
                client.join('#' + client.lon[i] + '_' + client.lat[j]);
            }
        }
    });
    /**
    ...
    client.on("insert", function(data, fn) {
        var handler = function(result) {
            ...
            var lon = raster.transform('lon', raster.rasterize('lon', data.lon, data.lon), raster.messageLevel('lon'));
            var lat = raster.transform('lat', raster.rasterize('lat', data.lat, data.lat), raster.messageLevel('lat'));
            client.broadcast.to('#' + lon.values[0] + '_' + lat.values[0]).emit('fragment', {
                id : handler.id,
                lon : data.lon,
                lat : data.lat,
                title : data.title,
