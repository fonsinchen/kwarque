...
var queries = {
    'getFragments'   : "SELECT id, x, y, title, text, owner, EXTRACT(EPOCH FROM updated) AS timestamp" +
                       " FROM fragment" +
                       " WHERE x > $1 AND x < $3 AND y > $2 AND y < $4",
    'insertFragment' : "INSERT INTO fragment (x, y, title, text, time, owner) VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP(0), $5) RETURNING id, EXTRACT(EPOCH FROM updated) AS timestamp",
        ...
        if (client.nick === null) {
            ...
            var handler = function(result) {
                ...
                tree.retrieve(data, function(clients) {
                    for (var key in clients) {
                        if (clients.hasOwnProperty(key) && client !== clients[key]) {
