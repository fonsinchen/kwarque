...
var queries = {
    'getFragments'   : "SELECT id, location[0] AS x, location[1] AS y, title, text FROM fragment WHERE location <@ BOX(POINT($1, $2), POINT($3, $4))",
...
var insertFragment = function(data, end) {
    query('insertFragment', [data.x, data.y, data.title, data.text], function(row) {
        end.id = row.id;
    }, end);
};
